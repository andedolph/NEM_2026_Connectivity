## Automation Communication Protocols Lab 1 - MQTT
...
This lab uses TwinCAT 3.1 and the TF6701 IoT Communication library. Please make sure to install the TF6701 package. We will also be using the [*MQTT Explorer*](https://mqtt-explorer.com/) client tool for testing. If you already have a favorite MQTT client interface, feel free to use that as well.
...
IF bSend THEN
  ...
  sendData.Message := 'Hello from NEM 2026!'; // put your message here!
...
```json
{ "Id": 1, "Message": "Hello from NEM 2026!" }
```
...
IF bSubscribe THEN
  ...
  fbMqttClient.Subscribe(sTopic:='#'); // listen for all nem_2026 messages
  bSubscribe := FALSE;
END_IF
...
>Note that we are also calling the `fbMessage.GetTopic()` method. We are subscribed to a wildcard path (nem_2026/**#**), but we can still get the exact topic to which the message was published.
...
And finally, declare a new structure instance to hold the received data, and revise the listening logic:
```js
